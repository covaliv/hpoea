...
constexpr std::string_view kFamily = "SelfAdaptiveDE";
constexpr std::string_view kImplementation = "pagmo::sade";
constexpr std::string_view kVersion = "2.x";

// pagmo::sade implements two self-adaptation variants:
...
ParameterSpace make_parameter_space() {
  ...
  ParameterDescriptor variant_adptv;
  variant_adptv.name = "variant_adptv";
  variant_adptv.type = ParameterType::Integer;
  variant_adptv.integer_range = hpoea::core::IntegerRange{1, 10};
  variant_adptv.default_value = static_cast<std::int64_t>(1);
  space.add_descriptor(variant_adptv);
...
std::size_t determine_generations(const ParameterSet &parameters,
                                  const Budget &budget,
                                  std::size_t population) {
  std::size_t generations = static_cast<std::size_t>(
      std::get<std::int64_t>(parameters.at("generations")));
  ...
  if (budget.function_evaluations.has_value()) {
    const std::size_t max_generations = budget.function_evaluations.value() /
...
void PagmoSelfAdaptiveDE::configure(const ParameterSet &parameters) {
...
OptimizationResult PagmoSelfAdaptiveDE::run(const core::IProblem &problem,
  ...
  try {
    const auto population_size = static_cast<std::size_t>(
        std::get<std::int64_t>(configured_parameters_.at("population_size")));
    const auto variant = static_cast<unsigned>(
        std::get<std::int64_t>(configured_parameters_.at("variant")));
    const auto variant_adptv = static_cast<unsigned>(
        std::get<std::int64_t>(configured_parameters_.at("variant_adptv")));
    ...
    const auto generations =
        determine_generations(configured_parameters_, budget, population_size);
    effective_parameters.insert_or_assign(
        "generations", static_cast<std::int64_t>(generations));

    pagmo::algorithm algorithm{pagmo::sade(static_cast<unsigned>(generations),
                                           variant, variant_adptv, ftol, xtol,
                                           memory)};
    pagmo::problem pg_problem{ProblemAdapter{problem}};

    const auto population_seed = static_cast<unsigned int>(
