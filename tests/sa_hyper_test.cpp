...
int main() {
    ...
    const char *run_flag = std::getenv("HPOEA_RUN_SA_TESTS");
    if (!run_flag || std::string_view{run_flag} != "1") {
        std::cout << "Skipping Simulated Annealing hyper optimizer test (set HPOEA_RUN_SA_TESTS=1 to enable)" << std::endl;
        return 0;
    }
    ...
    optimizer_overrides.emplace("iterations", static_cast<std::int64_t>(50));
    optimizer_overrides.emplace("ts", 10.0);
    optimizer_overrides.emplace("tf", 0.1);
    ...
    if (result.status != core::RunStatus::Success) {
        std::cerr << "Simulated Annealing optimization failed: " << result.message << '\n';
        return 1;
    }
