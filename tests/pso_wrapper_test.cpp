...
int main() {
...

    {
        ...
        test_cases.push_back(TestCase{
            "rastrigin",
            [] { return std::make_unique<wrappers::problems::RastriginProblem>(6); },
            std::move(params),
            budget,
            10.0});
    }
    ...
    try {
        ...
        invalid.emplace("omega", 2.0); // Out of range
        algorithm->configure(invalid);
        std::cerr << "Expected ParameterValidationError for invalid omega" << '\n';
        return 5;
